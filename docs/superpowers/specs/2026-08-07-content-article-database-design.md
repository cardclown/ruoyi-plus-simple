...
| `tag_ids` | `varchar(200)` | 否 | `''` | 逗号分隔的标签 ID，最多 10 个 |
...
|---:|---|---|
| 1 | 行业洞察 | `industry_insight` |
| 2 | 技术分享 | `technology_sharing` |
| 3 | 产品动态 | `product_updates` |
| 4 | 开源社区 | `open_source_community` |
...
前端请求和响应使用 `tagIds` 数组。后端保存文章时对标签 ID 去重并限制最多 10 个，验证分类和标签属于当前租户及对应字典类型，然后在同一事务内：
...
- 标签最多 10 个、标签去重和字典归属都在代码层校验，不建立数据库检查约束。
- 本期不支持恢复已删除文章。
...
数据库脚本放入 `script/sql/postgres/`，包含两张表、字段注释、两个字典类型和十四条字典数据。执行前先使用事务和遇错即停模式验证脚本；执行后检查表结构、主键、默认值和字典数量。脚本不得创建业务二级索引。
